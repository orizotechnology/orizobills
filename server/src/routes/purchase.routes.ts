...
import { getNextPurchaseNumber, getNextPurReturnNumber } from "../services/counter.service";

// =============================================================
// PURCHASE ROUTES — /api/purchases
// Fixes applied:
//  - DELETE: hard-deletes invoice (not just CANCELLED status)
//  - POST /returns: now persists return items to DB
//  - Invoice numbers use counter.service (consistent with sale routes)
//  - All multi-step DB ops wrapped in $transaction
// =============================================================
...
export async function purchaseRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/returns", async (req: FastifyRequest<{ Querystring: { page?: string; pageSize?: string } }>, reply) => {
    try {
      ...
      const [rows, total] = await Promise.all([
        req.prisma.purchaseReturn.findMany({
  ...
  fastify.get("/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply) => {
    try {
      const p = await req.prisma.purchaseInvoice.findUnique({
        where: { id: req.params.id },
        include: { items: true },
      });
      if (!p) return reply.status(HTTP_STATUS.NOT_FOUND).send(
        errorResponse("Not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND)
      );
  ...
  fastify.post("/", async (req, reply) => {
    ...
    if (!parse.success) return reply.status(HTTP_STATUS.BAD_REQUEST).send(
      errorResponse(parse.error.errors[0]?.message ?? "Validation failed", HTTP_STATUS.BAD_REQUEST, ERROR_CODES.VALIDATION_ERROR)
    );
    ...
    try {
      ...
      const taxAmt      = items.reduce((s, i) => s + i.taxAmount, 0);
      const totalAmt    = subtotal - discountAmt + taxAmt;

      // ── Wrap everything in a transaction ─────────────────
      const invoice = await req.prisma.$transaction(async (tx: typeof req.prisma) => {
        const inv = await tx.purchaseInvoice.create({
          data: {
            ...
            poDate:        rest.poDate ? new Date(rest.poDate) : null,
            poNumber:      rest.poNumber ?? null,
        ...
        // Update inventory stockIn
        for (const item of items) {
          if (item.productId) {
            await tx.inventoryItem.upsert({
              where:  { productId: item.productId },
              create: { productId: item.productId, openingStock: 0, stockIn: item.quantity, stockOut: 0, lowStockAlert: 5 },
              update: { stockIn: { increment: item.quantity } },
            });
          }
        }

        // Update supplier balance for Credit payments
        if (rest.supplierId && rest.paymentMethod === "Credit") {
          await tx.supplier.update({
            where: { id: rest.supplierId },
            data:  { balance: { increment: totalAmt } },
          });
        }
  ...
  // ── DELETE — hard delete (same as sale DELETE) ────────────
  fastify.delete("/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply) => {
    try {
      const inv = await req.prisma.purchaseInvoice.findUnique({
        where:   { id: req.params.id },
        include: {
          items: true,
      ...
      if (!inv) return reply.status(HTTP_STATUS.NOT_FOUND).send(
        errorResponse("Invoice not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND)
      );

      // Reverse inventory stockIn for each item and delete dependent returns first
      await req.prisma.$transaction(async (tx: typeof req.prisma) => {
        await tx.purchaseReturn.deleteMany({ where: { invoiceId: req.params.id } });
        ...
        for (const item of inv.items as any[]) {
          if (item.productId) {
            await tx.inventoryItem.update({
              where: { productId: item.productId },
              data:  { stockIn: { decrement: parseFloat(String(item.quantity)) } },
            }).catch(() => { /* ignore if inventory row missing */ });
          }
        }
        // Hard delete — cascade removes items automatically
        await tx.purchaseInvoice.delete({ where: { id: req.params.id } });
      });

      return reply.send(successResponse(null, "Purchase deleted"));
    } catch (err) {
      return reply.status(HTTP_STATUS.INTERNAL_ERROR).send(
        errorResponse(String(err), HTTP_STATUS.INTERNAL_ERROR, ERROR_CODES.DATABASE_ERROR)
      );
  ...
  fastify.post("/returns", async (req, reply) => {
    ...
    try {
      ...
      const ret = await req.prisma.$transaction(async (tx: typeof req.prisma) => {
        // Create return with items (fix: items are now persisted)
        const r = await tx.purchaseReturn.create({
          data: {
            returnNumber,
            invoiceId:    parse.data.invoiceId    ?? null,
            supplierId:   parse.data.supplierId   ?? null,
            supplierName: parse.data.supplierName,
            returnDate:   new Date(parse.data.returnDate),
        ...
        // Decrement stockIn for returned goods
        for (const item of parse.data.items) {
          if (item.productId) {
            await tx.inventoryItem.update({
              where: { productId: item.productId },
              data:  { stockIn: { decrement: item.quantity } },
            }).catch(() => {});
          }
        }
