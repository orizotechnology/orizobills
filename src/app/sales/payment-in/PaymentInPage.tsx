      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            return (
              <div key={c.label} style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 12, padding: "16px 18px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 10, background: c.bg, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>{c.icon}</div>
                  <div>
                ...
                {methods.length > 0 && (
                  <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px dashed #E2E8F0", display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {methods.map(([method, amount]) => {
                      ...
                      return (
                        <div key={method} style={{ display: "inline-flex", alignItems: "center", gap: 5, background: ms.bg, borderRadius: 8, padding: "4px 10px" }}>
                          ...
                          <span style={{ fontSize: 12, fontWeight: 800, color: ms.color }}>{fmtAmt(amount)}</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {grouped.map(([day, rows]) => {
            ...
            rows.forEach((r) => { dayByMethod[r.paymentMethod] = (dayByMethod[r.paymentMethod] ?? 0) + r.amount; });
            ...
            return (
              <div key={day} style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", background: isToday ? "rgba(249,115,22,0.04)" : "#F8FAFC", borderBottom: "1px solid #E2E8F0" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    {isToday && <span style={{ fontSize: 10, fontWeight: 700, background: "#F97316", color: "#fff", borderRadius: 20, padding: "1px 8px", letterSpacing: "0.05em" }}>TODAY</span>}
                    <span style={{ fontSize: 13, fontWeight: 700, color: "#0F172A" }}>{fmtDate(day)}</span>
                    <span style={{ fontSize: 12, color: "#94A3B8" }}>· {rows.length} transaction{rows.length !== 1 ? "s" : ""}</span>
                    {methodEntries.map(([method, amount]) => {
                      const ms = METHOD_STYLE[method] ?? METHOD_STYLE["Bank Transfer"];
                      return (
                        <span key={method} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, borderRadius: 20, padding: "2px 8px", background: ms.bg, color: ms.color }}>
                          {ms.icon} {method} {fmtAmt(amount)}
                        </span>
                      );
                    })}
                  </div>
                  <span style={{ fontSize: 14, fontWeight: 800, color: "#F97316", flexShrink: 0, marginLeft: 8 }}>{fmtAmt(dayTotal)}</span>
                </div>
                    ...
                    {rows.map((p, idx) => (
                      <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                        ...
                        <td style={{ padding: "11px 16px", width: 110 }}>
                          <code style={{ fontSize: 11, background: "#F1F5F9", borderRadius: 4, padding: "2px 7px", color: "#475569" }}>{p.paymentNumber}</code>
                        </td>
                        <td style={{ padding: "11px 12px", fontSize: 13, fontWeight: 500, color: "#1E293B" }}>{p.customerName}</td>
                        <td style={{ padding: "11px 12px", fontSize: 12, color: "#94A3B8" }}>{p.reference ?? (p.invoiceId ? "Invoice" : "—")}</td>
                        <td style={{ padding: "11px 12px" }}><MethodBadge method={p.paymentMethod} /></td>
                        <td style={{ padding: "11px 16px", textAlign: "right", fontSize: 14, fontWeight: 800, color: "#22C55E" }}>{fmtAmt(p.amount)}</td>
                      </motion.tr>
                    ))}
                  </tbody>
                </table>
...
.method-scroll::-webkit-scrollbar { display: none; }`}</style>
    </div>
  );
}
