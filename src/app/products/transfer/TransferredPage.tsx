...
import { ArrowUpRight, RefreshCw, AlertTriangle, Package } from "lucide-react";
import { http } from "@/lib/axios";

// =============================================================
// TRANSFERRED PAGE — products sent out from this branch
// Groups by destination branch so each branch is shown separately
// =============================================================
...
export default function TransferredPage() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    ...
    queryFn:  async () => {
      const res = await http.get<ApiResponse<{ data: StockTransfer[]; total: number }>>("/transfers/sent?pageSize=500");
  ...
  // Group by destination branch — each branch shown separately
  const grouped = useMemo(() => {
    const map: Record<string, { name: string; items: StockTransfer[] }> = {};
    transfers.forEach((t) => {
      if (!map[t.toBranchId]) map[t.toBranchId] = { name: t.toBranchName, items: [] };
      map[t.toBranchId].items.push(t);
    });
    return Object.entries(map).sort(([, a], [, b]) => a.name.localeCompare(b.name));
  }, [transfers]);

  // Summary: total quantity sent per branch
  const totalQty = transfers.reduce((s, t) => s + t.quantity, 0);

  const handleRefresh = async () => {
    await qc.invalidateQueries({ queryKey: ["transfers-sent"] });
    await refetch();
  };
  ...
  return (
    <div style={{ padding: "24px 28px", minHeight: "100%", background: "#F8FAFC" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: "#0F172A" }}>Products Transferred</div>
          <div style={{ fontSize: 13, color: "#94A3B8", marginTop: 2 }}>
            Stock sent out from this branch to other branches
          </div>
        </div>
      ...
      {transfers.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
          {[
            { label: "Total Transfers",      value: String(total),              color: "#F97316" },
            { label: "Branches Supplied",    value: String(grouped.length),    color: "#8B5CF6" },
            { label: "Total Units Sent",     value: totalQty.toFixed(0),        color: "#EF4444" },
          ].map((c) => (
            <div key={c.label} style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 10,
              padding: "14px 18px" }}>
      ...
      {isLoading && (
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0",
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 13, color: "#94A3B8" }}>Loading transfers…</span>
          </div>
        </div>
      )}
      ...
      {!isLoading && !isError && grouped.length === 0 && (
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0",
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <ArrowUpRight size={44} color="#E2E8F0" />
            <div style={{ fontWeight: 600, color: "#94A3B8" }}>No transfers sent yet</div>
            <div style={{ fontSize: 12, color: "#CBD5E1" }}>Use the Transfer page to send stock to other branches</div>
          </div>
        </div>
      )}

      {/* Grouped by destination branch — each branch in its own card */}
            ...
            <div key={branchId} style={{ background: "#fff", borderRadius: 12, border: "1px solid #E2E8F0", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#0F172A" }}>To: {name}</div>
                    <div style={{ fontSize: 12, color: "#94A3B8" }}>{items.length} transfer{items.length !== 1 ? "s" : ""}</div>
                  </div>
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#EF4444" }}>
                  {branchTotal.toFixed(0)} units sent
                </div>
              </div>

              {/* Transfer rows for this branch */}
                  ...
                  <AnimatePresence initial={false}>
                    {items.map((t, idx) => (
                      <motion.tr key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <td style={{ ...tdStyle, fontWeight: 700, color: "#EF4444" }}>{t.quantity}</td>
