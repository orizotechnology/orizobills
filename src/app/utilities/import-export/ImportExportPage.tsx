  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 860, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
        <div style={{ width: 40, height: 40, borderRadius: 10, background: "hsl(var(--primary) / 0.08)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <FileSpreadsheet size={20} color="hsl(var(--primary))" />
        </div>
        <div>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "hsl(var(--foreground))" }}>Import &amp; Export</h1>
          <p style={{ margin: 0, fontSize: 13, color: "hsl(var(--muted-foreground))" }}>Attach any Excel or CSV — columns are detected automatically, no mapping needed</p>
        </div>
      </div>

      <div style={{ display: "flex", gap: 4, background: "hsl(var(--muted))", borderRadius: 10, padding: 4, marginBottom: 24, width: "fit-content" }}>
...
function ImportPanel() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      {/* Module select */}
      <div style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 12, padding: "18px 22px" }}>
        ...
        <div style={{ position: "relative", width: 220, marginTop: 8 }}>
          <select value={module} onChange={(e) => changeModule(e.target.value as Module)}
            style={{ width: "100%", appearance: "none", padding: "9px 32px 9px 12px", borderRadius: 8, border: "1.5px solid hsl(var(--border))", fontSize: 14, color: "hsl(var(--foreground))", background: "hsl(var(--background))", cursor: "pointer", outline: "none" }}>
            {MODULES.map((m) => <option key={m}>{m}</option>)}
          </select>
        </div>
        <p style={{ margin: "10px 0 0", fontSize: 12, color: "hsl(var(--muted-foreground))" }}>
      ...
      {(phase === "idle" || phase === "error") && (
        <>
          <div
            ...
            style={{ background: "hsl(var(--card))", border: `2px dashed ${phase === "error" ? "hsl(var(--destructive) / 0.25)" : "hsl(var(--muted))"}`, borderRadius: 12, padding: "44px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: 12, cursor: "pointer", transition: "border-color 0.15s" }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "hsl(var(--primary))"; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = phase === "error" ? "hsl(var(--destructive) / 0.25)" : "hsl(var(--muted))"; }}
          >
            <Upload size={32} color={phase === "error" ? "hsl(var(--destructive))" : "hsl(var(--muted-foreground))"} />
            <div style={{ textAlign: "center" }}>
              <p style={{ margin: 0, fontSize: 15, fontWeight: 600, color: "hsl(var(--foreground))" }}>
                Drop your file here or <span style={{ color: "hsl(var(--primary))" }}>browse</span>
              </p>
          ...
          {errMsg && (
            <div style={{ background: "hsl(var(--destructive) / 0.06)", border: "1px solid hsl(var(--destructive) / 0.2)", borderRadius: 8, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8 }}>
              <AlertCircle size={15} color="hsl(var(--destructive))" />
              <span style={{ fontSize: 13, color: "hsl(var(--destructive))" }}>{errMsg}</span>
            </div>
          )}
        </>
      )}
      ...
      {/* Parsing spinner */}
      {phase === "parsing" && (
        <div style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 10, padding: "22px", display: "flex", alignItems: "center", gap: 12 }}>
          <Loader2 size={20} color="hsl(var(--primary))" style={{ animation: "spin 0.7s linear infinite" }} />
          <span style={{ fontSize: 14, color: "hsl(var(--muted-foreground))" }}>Reading <strong>{fileName}</strong>…</span>
        </div>
      )}
      ...
      {phase === "ready" && parsed && (
        <div style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 12, overflow: "hidden" }}>
          {/* Summary bar */}
          <div style={{ padding: "16px 22px", borderBottom: "1px solid hsl(var(--muted))", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontWeight: 700, fontSize: 15, color: "hsl(var(--foreground))" }}>{fileName}</div>
              <div style={{ fontSize: 13, color: "hsl(var(--muted-foreground))", marginTop: 2 }}>
                <strong style={{ color: "hsl(var(--primary))" }}>{parsed.totalDataRows}</strong> rows detected
                &nbsp;·&nbsp;
                <strong style={{ color: "hsl(var(--primary))" }}>{parsed.originalHeaders.length}</strong> columns identified
                {parsed.batches?.length ? (
                  <>&nbsp;·&nbsp;<strong style={{ color: "hsl(var(--primary))" }}>{parsed.batches.length}</strong> batch rows (Sheet 2)</>
                ) : null}
              </div>
            </div>
            <button onClick={reset} style={{ background: "none", border: "none", cursor: "pointer", color: "hsl(var(--muted-foreground))" }}>
              <X size={16} />
            </button>
          </div>

          {/* Detected columns → normalised keys */}
          <div style={{ padding: "14px 22px", borderBottom: "1px solid hsl(var(--muted))" }}>
            <p style={{ margin: "0 0 10px", fontSize: 12, fontWeight: 600, color: "hsl(var(--foreground))", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              Columns found in your file
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {parsed.originalHeaders.map((h, i) => (
                <div key={h} style={{ display: "flex", alignItems: "center", gap: 6, background: "hsl(var(--background))", border: "1px solid hsl(var(--border))", borderRadius: 6, padding: "4px 10px" }}>
                  <span style={{ fontSize: 12, color: "hsl(var(--foreground))" }}>{h}</span>
                  <span style={{ fontSize: 10, color: "hsl(var(--muted-foreground))" }}>→</span>
                  <span style={{ fontSize: 12, fontWeight: 600, color: "hsl(var(--primary))" }}>{parsed.normalisedHeaders[i]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ overflowX: "auto", maxHeight: 220 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {parsed.rows.slice(0, 5).map((row, idx) => (
                    <tr key={idx} style={{ borderBottom: "1px solid hsl(var(--muted))" }}>
                      <td style={{ ...tdSt, color: "hsl(var(--muted-foreground))" }}>{idx + 1}</td>
                      {parsed.normalisedHeaders.map((h) => (
                        <td key={h} style={tdSt}>{row[h] !== undefined ? String(row[h]) : <span style={{ color: "hsl(var(--muted))" }}>—</span>}</td>
                      ))}
                    </tr>
                ))}
              </tbody>
            ...
            {parsed.totalDataRows > 5 && (
                <div style={{ padding: "8px 14px", fontSize: 12, color: "hsl(var(--muted-foreground))", background: "hsl(var(--background))", textAlign: "center" }}>
          ...
          {/* Confirm */}
          <div style={{ padding: "14px 22px", borderTop: "1px solid hsl(var(--muted))", display: "flex", justifyContent: "flex-end", gap: 10 }}>
            <button onClick={reset} style={{ ...btnSt, background: "hsl(var(--card))", border: "1.5px solid hsl(var(--border))", color: "hsl(var(--foreground))" }}>Cancel</button>
            <button onClick={confirmImport} style={{ ...btnSt, background: "hsl(var(--primary))", border: "none", color: "hsl(var(--primary-foreground))" }}>
      ...
      {/* Importing spinner */}
      {phase === "importing" && (
        <div style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 10, padding: "28px", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
          <Loader2 size={32} color="hsl(var(--primary))" style={{ animation: "spin 0.7s linear infinite" }} />
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontSize: 13, color: "hsl(var(--muted-foreground))", marginTop: 4 }}>Please wait</div>
          </div>
        </div>
      )}
      ...
      {/* Done */}
      {phase === "done" && result && (
        <div style={{ background: "hsl(var(--success) / 0.06)", border: "1px solid hsl(var(--success) / 0.25)", borderRadius: 12, padding: "22px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <CheckCircle2 size={22} color="hsl(var(--success))" />
            <span style={{ fontSize: 16, fontWeight: 700, color: "hsl(var(--success))" }}>Import Complete</span>
          </div>
          <div style={{ display: "flex", gap: 32, marginBottom: result.errors.length > 0 ? 16 : 0 }}>
            <StatChip label="Inserted" value={result.inserted} color="hsl(var(--success))" />
            <StatChip label="Skipped"  value={result.skipped}  color="hsl(var(--warning))" />
          </div>
          {result.errors.length > 0 && (
            <div style={{ background: "hsl(var(--destructive) / 0.05)", borderRadius: 8, padding: "10px 14px", maxHeight: 130, overflowY: "auto", marginBottom: 12 }}>
              {result.errors.slice(0, 8).map((e, i) => <div key={i} style={{ fontSize: 12, color: "hsl(var(--destructive))", marginBottom: 2 }}>{e}</div>)}
              {result.errors.length > 8 && <div style={{ fontSize: 12, color: "hsl(var(--muted-foreground))" }}>…and {result.errors.length - 8} more</div>}
            </div>
          )}
          <button onClick={reset} style={{ ...btnSt, background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))", border: "none", marginTop: 4 }}>Import Another File</button>
        </div>
      )}

      {/* Template hint */}
      <div style={{ background: "hsl(var(--warning) / 0.06)", border: "1px solid hsl(var(--warning) / 0.2)", borderRadius: 10, padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 13, color: "hsl(var(--warning))" }}>
          Need a starting point? Download a sample template for <strong>{module}</strong>.
        </span>
        <button onClick={() => downloadTemplate(module)} style={{ ...btnSt, background: "hsl(var(--primary))", border: "none", color: "hsl(var(--primary-foreground))", display: "flex", alignItems: "center", gap: 6 }}>
...
function ExportPanel() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 12, padding: "20px 24px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 12 }}>
          {MODULES.map((m) => {
            ...
            return (
              <button key={m} onClick={() => toggle(m)} style={{ padding: "7px 16px", borderRadius: 20, border: `1.5px solid ${on ? "hsl(var(--primary))" : "hsl(var(--border))"}`, background: on ? "hsl(var(--primary) / 0.08)" : "hsl(var(--card))", color: on ? "hsl(var(--primary))" : "hsl(var(--foreground))", fontWeight: on ? 600 : 400, fontSize: 13, cursor: "pointer" }}>
                {m}
              </button>
            );
          })}
      ...
      <div style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 12, padding: "18px 24px" }}>
        <label style={lblSt}>Format</label>
        <div style={{ display: "flex", gap: 12, marginTop: 10 }}>
          {(["xlsx", "csv"] as const).map((f) => (
            <button key={f} onClick={() => setFmt(f)} style={{ padding: "7px 18px", borderRadius: 8, border: `1.5px solid ${fmt === f ? "hsl(var(--primary))" : "hsl(var(--border))"}`, background: fmt === f ? "hsl(var(--primary) / 0.08)" : "hsl(var(--card))", color: fmt === f ? "hsl(var(--primary))" : "hsl(var(--foreground))", fontWeight: fmt === f ? 600 : 400, fontSize: 13, cursor: "pointer", textTransform: "uppercase" }}>
              .{f}
            </button>
          ))}
        </div>
      ...
      <button onClick={handleExport} disabled={!selected.size || loading}
        style={{ ...btnSt, background: !selected.size ? "hsl(var(--muted))" : "hsl(var(--primary))", border: "none", color: "hsl(var(--primary-foreground))", fontSize: 14, padding: "12px 0", borderRadius: 10, cursor: !selected.size || loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
...
const lblSt: React.CSSProperties = { fontSize: 13, fontWeight: 600, color: "hsl(var(--foreground))" };
...
const thSt:  React.CSSProperties = { padding: "9px 12px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "hsl(var(--muted-foreground))", letterSpacing: "0.04em", whiteSpace: "nowrap" };
const tdSt:  React.CSSProperties = { padding: "7px 12px", fontSize: 12, color: "hsl(var(--foreground))", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" };
